...
// get all type of tea to add into the option the form 
...
router.post("/search", jsonParser, function (req, res) {
    ...
    let unitCondition = " AND (product_details.prod_unit = 'box' OR product_details.prod_unit = 'bag');"; //in case that user checked both of them
    let typCondition = "";
    //if user selects the particular type condition to select, it will be added to the query. Otherwise, it will not add any condition.
    if (typT != "all") { typCondition = " AND (tt_name = '"+typT+"')"; } 
    //check whether unit is the one user checked
    if (unitT == "bag") {
        unitCondition = " AND (product_details.prod_unit = 'bag');" //add it to the condition in query
    } else if (unitT == "box") {
        unitCondition = " AND (product_details.prod_unit = 'box');" //add it to the condition in query
    } 
    let qur = "SELECT product.prod_id, tt_name, product.prod_name, product_details.prod_price, product_details.prod_unit FROM typetea"
...
router.post("/teaorytype", jsonParser, function (req, res) {
    ...
    dbConnection.query("INSERT INTO TypeTea SET ? ", typInfo, function (error, results) {
        if (error) if (error.errno == 1062) {
            return res.send({ 
                error: true, message: error.message+" Please change the id"
            });
        } else {
            throw error;
        }
...
router.delete("/teaorytype", jsonParser, function (req, res) {
    ...
    dbConnection.query("DELETE FROM TypeTea WHERE tt_id = ?;", tid, function (error, results) {
        if (error) {
            if (error.errno == 1217) {
                return res.send({ 
                    error: true,  message: "This type is refered by existing product! Cannot delete!"
                });
            } else {  throw error; }
        }
        return res.send({ 
...
router.get("/teaoryitem/:id", jsonParser, function(req, res) {
    ...
    let q = "SELECT typetea.tt_id, tt_name, product.prod_id, product.prod_name, prod_status, prod_des, product_details.prod_price, product_details.prod_unit, prod_des, prod_logo, prod_img1, prod_img2" 
...
router.post("/searchuser", jsonParser, function (req, res) {
    ...
    let _email = (req.body.condition.user_email ? " AND (LOWER(user_email) LIKE '%"+req.body.condition.user_email.toLowerCase()+"%')" : "");
    let _username = (req.body.condition.login_user ? " AND (LOWER(login_user) LIKE '%"+req.body.condition.login_user.toLowerCase()+"%')" : "");
...
router.post("/teaoryuser", jsonParser, function (req, res) {
    ...
    dbConnection.query("INSERT INTO UserInfo  SET ? ", uinfo, function (error, results) {
        if (error) if (error.errno == 1062) {
            return res.send({ error: true, message: error.message+" Please change the id" });
        } else { throw error; }
